...
slots_per_epoch=$(curl -s http://localhost:3500/eth/v1/config/spec | jq -r .data.SLOTS_PER_EPOCH)
...
forge script script/integration/2_VerifyDepositNST.s.sol --skip-simulation \
    ...
    --evm-version cancun # required, otherwise you get EvmError: NotActtivated
