    ...
    UTXOGatewayStorage
{
     ...
     * @param pegoutTxId The client chain(e.g. Bitcoin) transaction ID that fulfills this peg-out request
        ...
        returns (PegOutRequest memory consumedRequest)
    {
        if (pegoutTxId == bytes32(0)) {
            revert Errors.InvalidPegoutTxId();
        }
        ...
        consumedRequest = pegOutRequests[clientChainId][requestNonce];
        ...
        // Map the Bitcoin txid to the request for future reference, this also means the peg-out request is processed
