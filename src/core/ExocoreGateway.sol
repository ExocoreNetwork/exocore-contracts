...
OAppUpgradeable
{
    ...
    function requestDeposit(uint32 srcChainId, uint64 lzNonce, bytes calldata payload) public onlyCalledFromThis {
        _handleRequest(srcChainId, lzNonce, payload, DEPOSIT_REQUEST_LENGTH, DEPOSIT_PRECOMPILE_ADDRESS, DEPOSIT_FUNCTION_SELECTOR, Action.REQUEST_DEPOSIT);
    }

    function requestWithdrawPrinciple(uint32 srcChainId, uint64 lzNonce, bytes calldata payload) public onlyCalledFromThis {
        _handleRequest(srcChainId, lzNonce, payload, WITHDRAW_PRINCIPLE_REQUEST_LENGTH, WITHDRAW_PRINCIPLE_PRECOMPILE_ADDRESS, WITHDRAW_PRINCIPLE_FUNCTION_SELECTOR, Action.REQUEST_WITHDRAW_PRINCIPLE_FROM_EXOCORE);
    }

    function requestWithdrawReward(uint32 srcChainId, uint64 lzNonce, bytes calldata payload) public onlyCalledFromThis {
        _handleRequest(srcChainId, lzNonce, payload, CLAIM_REWARD_REQUEST_LENGTH, CLAIM_REWARD_PRECOMPILE_ADDRESS, CLAIM_REWARD_FUNCTION_SELECTOR, Action.REQUEST_WITHDRAW_REWARD_FROM_EXOCORE);
    }

    function requestDelegateTo(uint32 srcChainId, uint64 lzNonce, bytes calldata payload) public onlyCalledFromThis {
        _handleRequestWithOperator(srcChainId, lzNonce, payload, DELEGATE_REQUEST_LENGTH, DELEGATION_PRECOMPILE_ADDRESS, DELEGATE_TO_THROUGH_CLIENT_CHAIN_FUNCTION_SELECTOR, Action.REQUEST_DELEGATE_TO);
    }

    function requestUndelegateFrom(uint32 srcChainId, uint64 lzNonce, bytes calldata payload) public onlyCalledFromThis {
        _handleRequestWithOperator(srcChainId, lzNonce, payload, UNDELEGATE_REQUEST_LENGTH, DELEGATION_PRECOMPILE_ADDRESS, UNDELEGATE_FROM_THROUGH_CLIENT_CHAIN_FUNCTION_SELECTOR, Action.REQUEST_UNDELEGATE_FROM);
    }

    function _handleRequest(uint32 srcChainId, uint64 lzNonce, bytes calldata payload, uint256 expectedLength, address precompileAddress, bytes4 functionSelector, Action action) private {
        _validatePayloadLength(payload, expectedLength, action);
        _decodeAndCall(precompileAddress, functionSelector, payload, srcChainId, lzNonce, action);
    }

    function _handleRequestWithOperator(uint32 srcChainId, uint64 lzNonce, bytes calldata payload, uint256 expectedLength, address precompileAddress, bytes4 functionSelector, Action action) private {
        _validatePayloadLength(payload, expectedLength, action);
        _decodeAndCallWithOperator(precompileAddress, functionSelector, payload, srcChainId, lzNonce, action);
    }

    function _decodeAndCall(address precompileAddress, bytes4 functionSelector, bytes calldata payload, uint32 srcChainId, uint64 lzNonce, Action action) private {
        ...
        (bool success, bytes memory responseOrReason) = precompileAddress.call(
            abi.encodeWithSelector(functionSelector, srcChainId, token, user, amount)
        );
    ...
    function _decodeAndCallWithOperator(address precompileAddress, bytes4 functionSelector, bytes calldata payload, uint32 srcChainId, uint64 lzNonce, Action action) private {
        bytes calldata token = payload[: 32];
        bytes calldata user = payload[32 : 64];
        ...
        (bool success,) = precompileAddress.call(
            abi.encodeWithSelector(functionSelector, srcChainId, lzNonce, token, user, operator, amount)
        );
    ...
    function _sendInterchainMsg(uint32 srcChainId, Action act, bytes memory actionArgs) internal whenNotPaused {
        ...
        MessagingReceipt memory receipt = _lzSend(srcChainId, payload, options, MessagingFee(fee.nativeFee, 0), exocoreValidatorSetAddress, true);
